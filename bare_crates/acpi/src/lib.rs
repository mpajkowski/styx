    ...
    H: AcpiHandler,
{
    ...
    pub fn ssdts(&self) -> SsdtIterator<H> {
        SsdtIterator {
            tables_phys_ptrs: self.tables_phys_ptrs(),
...
type TablesPhysPtrsIter<'t> =
    core::iter::Map<core::slice::Chunks<'t, u8>, fn(&[u8]) -> *const SdtHeader>;
...
unsafe fn read_table<H: AcpiHandler, T: AcpiTable>(
    ...
    // software issue).
    let header_mapping =
        unsafe { handler.map_physical_region::<SdtHeader>(address, mem::size_of::<SdtHeader>()) };
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        self.tables_phys_ptrs.find_map(|table_phys_ptr| {
            ...
            match unsafe { read_table::<_, Ssdt>(handler.clone(), table_phys_ptr as usize) } {
                ...
                Err(e) => {
                    log::warn!(
                        "Found invalid SSDT at physical address {:p}: {:?}",
                        table_phys_ptr,
                        e
                    );
